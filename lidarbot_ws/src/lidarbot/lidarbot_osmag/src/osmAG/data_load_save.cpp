    ...
    if(p.size()>1){
        ...
        for(auto it = p.begin()+1; it != p.end(); it++){
            ...
            if(passage_graph.pathedges_trj.find(edge_) != passage_graph.pathedges_trj.end()){
                ...
                if(graph.passages_.find(from) != graph.passages_.end()){
                    path_result.push_back(graph.passages_[from]->center_position);
                }
                if(graph.passages_.find(*it) != graph.passages_.end()){
                    path_result.push_back(graph.passages_[*it]->center_position);
                }
            }
            from = *it;
        }
        ...
        if(std::string(xml_node->Value())== "node"){
            ...
            if(tags_["osmAG:type"]=="area"){
                ...
                if(tags_.find("osmAG:parent") != tags_.end()){
                    has_parent = true;
                    parent_id = std::atol(tags_["osmAG:parent"].c_str());
                ...
                AreaId from = tags_.count("osmAG:from") ? std::atol(tags_["osmAG:from"].c_str()) : 0;
                AreaId to = tags_.count("osmAG:to") ? std::atol(tags_["osmAG:to"].c_str()) : 0;//check the node if is exist eg. wayid=0;
...
#ifndef OSMAG_LIBRARY_BUILD
int main(int argc, char * argv[]){
    AreaGraph graph;
    std::string file_path = "../data/show/sist_f2_V2.osm";
    if (argc > 1) {
        file_path = argv[1];
    }
    bool sucess_read = Parsing_Osm2AreaGraph(graph, file_path.c_str());

    if(!sucess_read){
        printf("Failed to parse OSMAG file: %s\n", file_path.c_str());
        return 1;
    }else{
        printf("Parsed OSM-AG XML successfully from: %s\n", file_path.c_str());
    }
    ...
    printf("Rendering 2D Grid Submaps for Areas...\n");
    int rendered_count = 0;
    for (auto& pair : graph.areas_) {
        AreaId aid = pair.first;
        InitOccupancyMap_Area(graph, aid, 0.1);
        rendered_count++;
    }
    printf("Successfully rendered %d 2D Grid Submaps!\n", rendered_count);
    ...
    if(map_compare.empty()){
        map_compare = img.clone();
    }
    if(map_compare.type() == CV_8UC1){
        cv::cvtColor(map_compare, map_compare, cv::COLOR_GRAY2BGR);
    }

    if (!graph.passages_.empty()) {
        auto it = graph.passages_.begin();
        PassageId start_id = it->first;
        std::advance(it, std::min<size_t>(45, graph.passages_.size() - 1));
        PassageId end_id = it->first;
        printf("--- Stage 4 Global Path Planning Execution ---\n");
        printf("Planning route from Passage %ld to Passage %ld...\n", start_id, end_id);
        fflush(stdout);
        std::vector<Eigen::Vector3d> path_result;
        PlanInPathGraph(graph, start_id, end_id, path_result);
        printf("Path result: %zu waypoints\n", path_result.size());
        fflush(stdout);

        // Use auto-scaled visualization (fixes off-screen coordinate bug in PlotPath)
        PlotGlobalPath(graph, path_result, "show_path.png");
        printf("Global Path Planning complete!\n");
        fflush(stdout);
    }
