...
namespace osm_ag{
    inline void ComputeCenter_Area(AreaGraph& graph, AreaId area_id){
    ...
    inline void InitOccupancyMap_Area(AreaGraph& graph, AreaId area_id, const double resolution = 0.2, const int offset = 4){
        ...
        cv::drawContours(area_mat,contour_pts,-1,cv::Scalar(255),cv::FILLED);//空闲区域是白色的
        ...
        for(auto passage : graph.areas_[area_id]->passageids){
            ...
            cv::circle(area_mat,point_m,4,cv::Scalar(255),cv::FILLED);
        ...
        std::string mat_name = "show_area_" + std::to_string(std::abs(area_id)) + ".png";
        cv::imwrite(mat_name, area_mat);
